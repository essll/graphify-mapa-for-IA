...
```
 ...
 /graphify config                                      # show current configuration
 /graphify config --set <key>=<value>                  # set a config value (e.g. GRAPHIFY_QUERY_DEFAULT_DEPTH=5)
 /graphify config --reset                              # reset all config to defaults
 /graphify cache stats                                 # show persistent cache statistics
 /graphify cache clear                                 # clear all persistent caches
 /graphify cache clear --namespace trigram_index       # clear specific cache namespace
```
